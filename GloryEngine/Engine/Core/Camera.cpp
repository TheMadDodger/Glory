...
namespace Glory
{
	Camera::Camera(uint32_t width, uint32_t height)
		: m_Resolution(width, height), m_IsOrtho(false),
		m_IsInUse(true), m_View(1.0f), m_Projection(1.0f),
		m_ClearColor(glm::vec4(0.0f)), m_Priority(0), m_LayerMask(0),
		m_Near(0.0f), m_Far(0.0f), m_HalfFOV(60.0f),
		m_ViewOffset(glm::identity<glm::mat4>())
	{

	}

	bool Camera::SetResolution(uint32_t width, uint32_t height)
	{
		if (m_Resolution.x == width && m_Resolution.y == height) return false;
	...
	void Camera::SetPerspectiveProjection(uint32_t width, uint32_t height, float halfFOV, float near, float far, bool force)
	{
	...
	void Camera::SetOrthographicProjection(float width, float height, float near, float far, bool force)
	{
