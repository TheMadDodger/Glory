...
namespace Glory
{
    ...
    {
        const bool isActive = pRegistry->GetTypeView<MeshRenderer>()->IsActive(entity) &&
            pRegistry->GetEntityView(entity)->IsActive();

        if (isActive && pComponent.m_RenderStatic && !pComponent.m_WasSubmittedForStatic)
            MeshRenderSystem::OnEnable(pRegistry, entity, pComponent);
        else if ((!isActive || !pComponent.m_RenderStatic) && pComponent.m_WasSubmittedForStatic)
            MeshRenderSystem::OnDisable(pRegistry, entity, pComponent);
    }
