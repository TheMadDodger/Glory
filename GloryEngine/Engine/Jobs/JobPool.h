...
namespace Glory::Jobs
{
	...
	protected:
		JobPoolBase(size_t poolID, size_t numJobsPerThread)
			: m_PoolID(poolID), m_NumJobsPerThread(numJobsPerThread) {}
		virtual ~JobPoolBase() {}

		virtual void Kill() = 0;

		const size_t NumHardwareThreads() const;
	...
	private:
		...
		void Initialize()
		{
			...
			const size_t hardwareThreads = NumHardwareThreads();
