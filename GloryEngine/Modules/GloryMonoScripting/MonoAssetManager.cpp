...
namespace Glory
{
	...
	MonoMethod* MonoAssetManager::m_pClearCacheMethod = nullptr;
	...
	void MonoAssetManager::Initialize(MonoImage* pImage)
	{
		...
		pMethodDesc = mono_method_desc_new(".::Clear()", false);
		m_pClearCacheMethod = mono_method_desc_search_in_class(pMethodDesc, m_pAssetManagerClass);
		mono_method_desc_free(pMethodDesc);
	}

	void MonoAssetManager::Cleanup()
	{
		MonoObject* pExcept;
		mono_runtime_invoke(m_pClearCacheMethod, nullptr, nullptr, &pExcept);
		m_AssetCache.clear();
	}
